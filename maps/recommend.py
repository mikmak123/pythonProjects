...
def find_centroid(cluster):
    ...
    meanx = mean([x for x,y in locations])
    meany = mean([y for x,y in locations])
    return [meanx, meany]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    "*** YOUR CODE HERE ***"
    meanx = mean(xs)
    Sx = [(x - meanx) for x in xs]
    Sxx = sum(x**2 for x in Sx )


    meany = mean(ys)
    Sy = [(y - meany)for y in ys]
    Syy = sum(y ** 2 for y in  Sy )


    S = zip(Sx, Sy)
    Sxy = sum( [x*y for x,y in S] )
...
def feature_set():
    """Returns a sequence of feature functions."""
